package com.example.retrofitbair.presentetion

import android.os.Bundle
import androidx.appcompat.app.AppCompatActivity
import androidx.navigation.findNavController
import androidx.navigation.ui.setupWithNavController
import com.example.retrofitbair.R
import com.example.retrofitbair.presentetion.adapter.PhoneAdapter
import com.example.retrofitbair.databinding.ActivityMainBinding
import com.google.android.material.bottomnavigation.BottomNavigationView

class MainActivity : AppCompatActivity() {
    private lateinit var adapter: PhoneAdapter
    lateinit var binding: ActivityMainBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityMainBinding.inflate(layoutInflater)
        setContentView(binding.root)

        val bottomNavigationMenu: BottomNavigationView = findViewById(R.id.bottomAppBar)
        val navController = findNavController(R.id.mainFragment)
        bottomNavigationMenu.setupWithNavController(navController)



    }
}
